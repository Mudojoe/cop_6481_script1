...
#   input : graoh - this is the networkx object that contains all
...
#       this layout algorithm will position nodes in the graph in an aesthetically pleasing way.
    ...
    nodes = nx.draw_networkx_nodes(graph, pos, node_size=[v * 300 for v in degrees], alpha=0.9)
    # create lablels for each node to be added to the graph
    labels = {node: node for node in graph.nodes()}
    nx.draw_networkx_labels(graph, pos, labels, font_size=14)

    # differentualate Normal edges from fraud edges
    normal_edges = [(u, v) for u, v in graph.edges() if (u, v) not in fraud_edges]
    # label the non-fraud edges
    normal_edge_labels = {(u, v): f"${graph[u][v]['amount']:.2f}" for u, v in normal_edges}
    # use the graph , the pos calculated above, draw thw non-fraud edges with a blue color
    nx.draw_networkx_edges(graph, pos, edgelist=normal_edges, edge_color='blue', width=1.0)


    # now draw the lable for the non-fraud edges
    # place the label for the amount slightly above the line for better visability
    edge_label_pos_offset = -0.075
    for (u, v), label in normal_edge_labels.items():
        x = (pos[u][0] + pos[v][0]) / 2  # Midpoint x-coordinate
        y = (pos[u][1] + pos[v][1]) / 2 + edge_label_pos_offset  # Midpoint y-coordinate + offset
        plt.text(x, y, label, color='blue', ha='center', va='bottom')  # Adjust the vertical alignment

    # Now draw the Fraud edges
    # Get the list of labels for the fraud edges
    fraud_edge_labels = {(u, v): f"${graph[u][v]['amount']:.2f}" for u, v in fraud_edges}
    # draw the fraud edges,m slightly thicket and with a red color
    nx.draw_networkx_edges(graph, pos, edgelist=fraud_edges, edge_color='red', width=2.0)
    ...
    for (u, v), label in fraud_edge_labels.items():
        x = (pos[u][0] + pos[v][0]) / 2  # Midpoint x-coordinate
        y = (pos[u][1] + pos[v][1]) / 2 + edge_label_pos_offset  # Midpoint y-coordinate + offset
        plt.text(x, y, label, color='red', ha='center', va='bottom')  # Adjust the vertical alignment
    # remove axis
    plt.axis('off')
    # render the graph
    plt.show()
...
#   input : graoh - this is the networkx object that contains all
#                   the nodes and edges for from our transactions.cvs file
#           threshold - any transaction that exceeds this ammount will be flagged as
...
def fraud_detection(graph, threshold=500.00):
    ...
    #       than the threshold
    fraud_edges = [(u, v) for u, v in graph.edges() if graph[u][v]['amount'] > threshold]
    ...
    for sender, receiver in fraud_edges:
        ...
        print(f"Potential Fraud Detected (Amount Exceeds $500.00): {sender} -> {receiver}, Amount: ${amount:.2f}")
...
# #####
# the main function
#       step 1 : read in the transactions.cvs file and contruct the
...
#                   threshold ammount. print out this list.
#       step 3 : visualize the network using matplotlib.pyplot
#
#
